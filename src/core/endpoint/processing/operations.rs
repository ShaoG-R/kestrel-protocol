...
use crate::{
    core::{
        endpoint::{
            core::frame::create_path_response_frame,
            lifecycle::ConnectionLifecycleManager,
            processing::traits::{EndpointOperations, ProcessorOperations},
            types::state::ConnectionState,
        },
        reliability::{UnifiedReliabilityLayer, logic::congestion::traits::CongestionController},
    },
    error::Result,
...
impl<T: Transport, C: CongestionController> EndpointOperations<C> for Endpoint<T, C> {
    ...
    fn complete_path_validation(&mut self, success: bool) -> Result<()> {
        self.lifecycle_manager_mut()
            .complete_path_validation(success)
    }
    ...
    async fn cancel_connection_timeout(&mut self) {
        let cancelled = self
            .timing
            .cancel_timer(&crate::core::endpoint::timing::TimeoutEvent::ConnectionTimeout)
            .await;
...
impl<T: Transport, C: CongestionController> ProcessorOperations<C> for Endpoint<T, C> {
    async fn process_ack_and_get_retx_frames(
        ...
        let ack_result = self
            ...
            .handle_ack_comprehensive(recv_next_seq, sack_ranges_converted, now, &context)
            .await;

        Ok(ack_result.frames_to_retransmit)
    }

    async fn receive_data_and_maybe_ack(&mut self, seq: u32, payload: Bytes) -> Result<()> {
        // 接收数据并检查是否需要立即发送 ACK
...
mod tests {
    ...
    use crate::{
        config::Config,
        core::{endpoint::types::state::ConnectionState, test_utils::MockTransport},
    ...
    async fn test_processor_operations_ack_processing() {
        ...
        // 测试 ACK 处理
        let result = endpoint
            .process_ack_and_get_retx_frames(1, vec![(1, 5), (10, 15)], Instant::now())
            .await;
