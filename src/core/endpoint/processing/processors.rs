...
pub trait UnifiedFrameProcessor<T: Transport> {
    ...
    fn validate_frame(frame: &Frame) -> Result<()> {
        if Self::can_handle(frame) {
            ...
            Err(crate::error::Error::InvalidFrame(format!(
                "Frame type not supported by {}",
                Self::name()
            )))
        }
    }
    ...
    pub async fn route_frame<T: Transport>(
        ...
        match ProcessorType::from_frame(&frame) {
            Some(ProcessorType::Push) => {
                ...
                <PushProcessor as UnifiedFrameProcessor<T>>::process_frame(
                    endpoint, frame, src_addr, now,
                )
                .await
            }
            Some(ProcessorType::Ack) => {
                ...
                <AckProcessor as UnifiedFrameProcessor<T>>::process_frame(
                    endpoint, frame, src_addr, now,
                )
                .await
            }
            Some(ProcessorType::Connection) => {
                ...
                <ConnectionProcessor as UnifiedFrameProcessor<T>>::process_frame(
                    endpoint, frame, src_addr, now,
                )
                .await
            }
            Some(ProcessorType::Path) => {
                ...
                <PathProcessor as UnifiedFrameProcessor<T>>::process_frame(
                    endpoint, frame, src_addr, now,
                )
                .await
            }
...
impl FrameProcessingContext {
    ...
    pub fn new(endpoint: &dyn ProcessorOperations, src_addr: SocketAddr, now: Instant) -> Self {
        Self {
            now,
            src_addr,
