...
impl UnifiedTimeoutScheduler {
    ...
    pub fn calculate_unified_deadline(&mut self, layers: &[&dyn TimeoutLayer]) -> Instant {
        ...
        let unified_deadline = all_deadlines
            .into_iter()
            .min()
    ...
    fn predict_next_timeout(&self, now: Instant) -> Option<Instant> {
        ...
        let recent_patterns: Vec<_> = self
            .timeout_patterns
            .iter()
        ...
        for window in recent_patterns.windows(2) {
            let interval = window[1]
                .timestamp
                .saturating_duration_since(window[0].timestamp);
    ...
    pub fn cleanup_expired_data(&mut self, now: Instant) {
        ...
        self.timeout_patterns
            .retain(|pattern| pattern.timestamp > cutoff);

        // 清理截止时间映射中的过期条目
        // Clean up expired entries in deadline mapping
        let expired_keys: Vec<_> = self
            .next_check_deadlines
            .range(..now)
            .map(|(k, _)| *k)
            .collect();
