...
use super::super::{
    data::buffer_stores::{PacketOrFin, ReceiveBufferStore, SendBufferStore},
    logic::packetization_processor::{
        PacketizationContext, PacketizationProcessor, PacketizationResult,
...
impl BufferCoordinator {
    ...
    pub fn new(config: &Config) -> Self {
        Self {
            ...
            receive_buffer: ReceiveBufferStore::new(
                config.reliability.recv_buffer_capacity_packets,
            ),
    ...
    pub fn receive_packet(&mut self, sequence_number: u32, payload: Bytes) -> bool {
        let accepted = self
            .receive_buffer
            .store_packet(sequence_number, payload.clone());
    ...
    pub fn receive_fin(&mut self, sequence_number: u32) -> bool {
        ...
        trace!(seq = sequence_number, accepted = accepted, "FIN received");
    ...
    pub fn get_statistics(&self) -> BufferCoordinatorStats {
        ...
        BufferCoordinatorStats {
            send_buffer_utilization: if status.send_buffer_status.total_capacity > 0 {
                (status.send_buffer_status.used_bytes as f64
                    / status.send_buffer_status.total_capacity as f64)
                    * 100.0
            } else {
                0.0
            },
            receive_buffer_utilization: if status.receive_buffer_status.total_capacity > 0 {
                (status.receive_buffer_status.used_slots as f64
                    / status.receive_buffer_status.total_capacity as f64)
                    * 100.0
            } else {
                0.0
            },
