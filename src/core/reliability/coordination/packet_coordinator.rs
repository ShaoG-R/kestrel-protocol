...
impl PacketCoordinator {
    ...
    pub async fn add_packet(&mut self, frame: &Frame, now: Instant, rto: Duration) -> bool {
        if let Some(seq) = frame.sequence_number() {
            if let Some(frame_info) = RetransmissionFrameInfo::from_frame(frame) {
                ...
                // 调度重传定时器
                match self.timer_event_handler.schedule_retransmission_timer(&mut self.store, seq, rto).await {
                    Ok(timer_id) => {
                        trace!(
                            seq = seq,
                            timer_id = timer_id,
                    ...
                    Err(e) => {
                        warn!(seq = seq, error = e, "Failed to schedule timer for packet, but keeping packet");
                        // 即使定时器失败，仍然保留数据包
                        return true;
                    }
                }
    ...
    pub async fn process_ack_comprehensive(
        ...
        if !sack_result.fast_retx_candidates.is_empty() {
            // 更新快速重传候选状态
            self.sack_processor.update_fast_retx_candidates(&mut self.store, &sack_result.fast_retx_candidates);
        ...
        if let Some(seq) = timer_result.sequence_number {
            if timer_result.needs_retransmission_handling {
                ...
                if !retx_decision.packets_to_drop.is_empty() {
                    // 清理达到最大重传次数的数据包
                    self.retransmission_decider.cleanup_dropped_packets(&mut self.store, &retx_decision.packets_to_drop);
                }
                
                if !retx_decision.packets_to_reschedule.is_empty() {
                    // 重新调度需要继续重传的数据包
                    for &reschedule_seq in &retx_decision.packets_to_reschedule {
                        if let Err(e) = self.timer_event_handler.reschedule_retransmission_timer(&mut self.store, reschedule_seq, rto).await {
                            warn!(seq = reschedule_seq, error = e, "Failed to reschedule timer after retransmission");
                        }
                    }
                }
    ...
    pub async fn check_rto_retransmission(
        ...
        if !retx_decision.packets_to_reschedule.is_empty() {
            for &seq in &retx_decision.packets_to_reschedule {
                if let Err(e) = self.timer_event_handler.reschedule_retransmission_timer(&mut self.store, seq, rto).await {
                    warn!(seq = seq, error = e, "Failed to reschedule timer after RTO check");
                }
            }
        }
    ...
    pub async fn cleanup_all(&mut self) {
        // 清理所有定时器
        self.timer_event_handler.cleanup_all_timers(&mut self.store).await;
    ...
    pub fn get_statistics(&self) -> PacketCoordinatorStats {
        PacketCoordinatorStats {
            ...
            needs_retx_count: self.retransmission_decider.get_retx_needed_count(&self.store),
            fast_retx_candidates: self.retransmission_decider.get_fast_retx_candidate_count(&self.store),
