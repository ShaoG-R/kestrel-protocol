...
use crate::core::reliability::logic::congestion::{
    CongestionController, CongestionDecision, CongestionState,
};
...
pub trait ConfigurableFlowControlCoordinator<C: CongestionController>:
    FlowControlCoordinator<C>
{
    /// 配置类型
    /// Configuration type
    type Config: Clone;
...
pub trait AdvancedFlowControlCoordinator<C: CongestionController>:
    FlowControlCoordinator<C>
{
    /// 处理网络状态变化
    /// Handle network condition change
    fn handle_network_condition_change(
        &mut self,
