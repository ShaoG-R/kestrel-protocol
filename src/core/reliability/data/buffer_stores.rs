...
impl SendBufferStore {
    ...
    pub fn write_data(&mut self, data: Bytes) -> usize {
        ...
        trace!(
            ...
            total_size = self.stream_buffer_size,
            "Data written to send buffer store"
        );
        bytes_to_write
    }
    ...
    pub fn extract_chunk(&mut self, max_size: usize) -> Option<Bytes> {
        ...
        trace!(
            extracted = chunk.len(),
            remaining_size = self.stream_buffer_size,
            "Data extracted from send buffer store"
        );
        Some(chunk)
    }
        ...
        match self.received.entry(sequence_number) {
            Entry::Vacant(entry) => {
                ...
                trace!(
                    seq = sequence_number,
                    payload_len = len,
                    "Packet stored in receive buffer"
                );
                true
            }
            Entry::Occupied(_) => {
