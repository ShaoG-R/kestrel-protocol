...
impl InFlightPacketStore {
    ...
    pub fn batch_remove_packets(&mut self, sequences: &[u32]) -> Vec<(u32, InFlightPacket)> {
        ...
        debug!(
            ...
            "Batch removed packets from store"
        );
        removed
    }
    ...
    pub fn set_timer_mapping(&mut self, timer_id: ActorTimerId, seq: u32) -> bool {
        ...
        if let Some(packet) = self.packets.get_mut(&seq) {
            ...
            trace!(
                seq = seq,
                timer_id = timer_id,
                "Timer mapping set successfully"
            );
            true
        } else {
            debug!(
                seq = seq,
                timer_id = timer_id,
                "Cannot set timer mapping: packet not found"
            );
            false
        }
    }
    ...
    pub fn get_packets_by_state(&self, state: PacketState) -> Vec<(u32, &InFlightPacket)> {
        if let Some(seq_set) = self.state_index.get(&state) {
            seq_set
                .iter()
                .filter_map(|&seq| self.packets.get(&seq).map(|packet| (seq, packet)))
                .collect()
        } else {
            Vec::new()
        }
    ...
    pub fn clear(&mut self) {
        debug!(
            ...
            "Clearing all packets from store"
        );
        self.packets.clear();
