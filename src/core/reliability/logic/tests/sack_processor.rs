...
use crate::core::reliability::logic::{SackProcessResult, SackProcessor};
use crate::packet::{frame::FrameType, sack::SackRange};
use bytes::Bytes;
...
fn test_empty_sack_ranges() {
    ...
    let result = processor.process_sack(&mut store, 5, &[], now);
...
fn test_invalid_sack_ranges() {
    ...
    let result = processor.process_sack(&mut store, 1, &sack_ranges, now);
...
fn test_fast_retx_detection_basic() {
    ...
    let sack_ranges = vec![SackRange { start: 3, end: 6 }];
...
fn test_fast_retx_detection_threshold() {
    ...
    let sack_ranges = vec![SackRange { start: 3, end: 5 }];
...
fn test_mixed_scenario() {
    ...
    // 累积确认1-3，SACK确认6-8
    let sack_ranges = vec![SackRange { start: 6, end: 8 }];
...
fn test_performance_many_sack_ranges() {
    ...
    let _result = processor.process_sack(&mut store, 1, &sack_ranges, now);
...
fn test_empty_store() {
    ...
    let sack_ranges = vec![SackRange { start: 1, end: 5 }];

    let result = processor.process_sack(&mut store, 10, &sack_ranges, now);
