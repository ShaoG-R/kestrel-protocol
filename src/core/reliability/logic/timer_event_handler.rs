...
impl TimerEventHandler {
    ...
    pub fn handle_timer_timeout(
        ...
        if let Some(seq) = store.find_sequence_by_timer(timer_id) {
            ...
            warn!(timer_id = timer_id, "Timer timeout for unknown timer ID");
    ...
    pub async fn batch_reschedule_timers(
        ...
        let cancelled_count = self
            .batch_cancel_retransmission_timers(store, sequences)
            .await;
        ...
        for chunk in sequences.chunks(MAX_CONCURRENT) {
            ...
            for task in tasks {
                match task.await {
                    Ok((seq, timer_result)) => match timer_result {
                        Ok(timer_id) => {
                            store.set_timer_mapping(timer_id, seq);
                    ...
                    Err(join_error) => {
                        ...
                        debug!(
                            seq = "unknown",
                            error = error_msg,
                            "Failed to join timer scheduling task"
                        );
                    }
                }
            }
    ...
    pub async fn cleanup_all_timers(&self, store: &mut InFlightPacketStore) {
        ...
        if !all_sequences.is_empty() {
            let cancelled_count = self
                .batch_cancel_retransmission_timers(store, &all_sequences)
                .await;
            debug!(
                cancelled_count = cancelled_count,
                "Cleaned up all retransmission timers"
            );
        }
    }
