...
use crate::socket::{
    handle::TransportReliableUdpSocket,
...
async fn test_rebind_with_existing_connections() {
    ...
    let (server_socket, mut server_listener) = TransportReliableUdpSocket::<UdpTransport>::bind("127.0.0.1:0".parse().unwrap()).await.unwrap();
    let server_addr = server_socket.local_addr().await.unwrap();
    
    let (client_socket, _) = TransportReliableUdpSocket::<UdpTransport>::bind("127.0.0.1:0".parse().unwrap()).await.unwrap();
    ...
    let (client_stream_result, server_accept_result) = tokio::try_join!(connect_task, accept_task).unwrap();
    ...
    server_socket.rebind("127.0.0.1:0".parse().unwrap()).await.unwrap();
    ...
    let (new_client_socket, _) = TransportReliableUdpSocket::<UdpTransport>::bind("127.0.0.1:0".parse().unwrap()).await.unwrap();
...
async fn test_client_rebind_scenario() {
    ...
    // 客户端rebind
    // Client rebinds
    client_socket.rebind("127.0.0.1:0".parse().unwrap()).await.unwrap();
    let new_client_addr = client_socket.local_addr().await.unwrap();
    
    info!("客户端从 {} rebind到 {}", original_client_addr, new_client_addr);
...
async fn test_simple_rebind_new_connections() {
    ...
    // 2. 创建第一个客户端并建立连接
    let (client1, _) = TransportReliableUdpSocket::<UdpTransport>::bind("127.0.0.1:0".parse().unwrap()).await.unwrap();
    
    let connect_task = {
        let client = client1.clone();
        tokio::spawn(async move {
            client.connect(original_server_addr).await
        })
    };
    ...
    // 4. 尝试从新客户端连接到新地址
    let (client2, _) = TransportReliableUdpSocket::<UdpTransport>::bind("127.0.0.1:0".parse().unwrap()).await.unwrap();
...
async fn test_rebind_impact_on_existing_connection_data() {
    ...
    let connect_task = {
        let client_socket = client_socket.clone();
        tokio::spawn(async move {
            client_socket.connect(original_server_addr).await
        })
    };
    ...
    // 5. 测试新连接
    let (new_client, _) = TransportReliableUdpSocket::<UdpTransport>::bind("127.0.0.1:0".parse().unwrap()).await.unwrap();
