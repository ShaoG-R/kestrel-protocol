...
// 重新导出 pool.rs 中的优秀设计
pub use super::pool::{TimerEventPool, PoolConfig, PoolStats};
    ...
    E: Default + Clone,
{
    ...
    pub fn create_events_optimized(&self, requests: &[(u32, E)]) -> Vec<TimerEventData<E>> {
        ...
        self.processed_count.fetch_add(batch_size, Ordering::Relaxed);
    ...
    pub fn get_performance_stats(&self) -> (usize, usize, usize, f64, f64) {
        ...
        (processed, pool_stats.current_size, 0, 0.0, large_batch_ratio)
    }
}
...
mod tests {
    ...
    fn test_optimized_batch_processor() {
        ...
        // 大批量处理
        let large_requests: Vec<_> = (0..50)
            .map(|i| (i, TimeoutEvent::IdleTimeout))
            .collect();
