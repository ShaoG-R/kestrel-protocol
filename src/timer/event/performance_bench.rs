...
impl ZeroCopyBenchmarker {
        ...
        for &dispatcher_count in &self.config.dispatcher_counts {
            for &batch_size in &self.config.batch_sizes {
                let config_name = format!(
                    "BatchDispatcher_{}disp_{}batch",
                    dispatcher_count, batch_size
                );

                let dispatcher =
                    ZeroCopyBatchDispatcher::<TimeoutEvent>::new(256, dispatcher_count, 1000);
    ...
    pub fn bench_smart_vs_optimized_vs_original(&mut self) {
        ...
        for &batch_size in &self.config.batch_sizes {
            ...
            // 记录结果
            let result_orig =
                BenchResult::new(config_name_orig, total_dispatched_orig, duration_orig);
            let result_opt = BenchResult::new(config_name_opt, total_dispatched_opt, duration_opt);
            let result_smart =
                BenchResult::new(config_name_smart, total_dispatched_smart, duration_smart);
            ...
            let improvement_opt = if duration_orig.as_nanos() > 0 {
                (duration_orig.as_nanos() as f64 - duration_opt.as_nanos() as f64)
                    / duration_orig.as_nanos() as f64
                    * 100.0
            } else {
                0.0
            };
            ...
            let improvement_smart = if duration_orig.as_nanos() > 0 {
                (duration_orig.as_nanos() as f64 - duration_smart.as_nanos() as f64)
                    / duration_orig.as_nanos() as f64
                    * 100.0
            } else {
                0.0
    ...
    pub fn bench_smart_end_to_end_workflow_optimized(&mut self) {
        ...
        for &batch_size in &self.config.batch_sizes {
            ...
            println!(
                "    - 平均利用率: {:.1}%",
                stats.average_slot_utilization * 100.0
            );
            println!();
    ...
    pub fn bench_memory_pool_optimization(&mut self) {
        ...
        for &batch_size in &self.config.batch_sizes {
            ...
            // 打印内存池统计
            let (processed, created, reused, reuse_rate, large_batch_ratio) =
    ...
    pub fn print_performance_analysis(&self) {
        ...
        for (component, results) in component_results {
            ...
            let avg_ns_per_event: f64 =
                results.iter().map(|r| r.nanoseconds_per_event).sum::<f64>() / results.len() as f64;
            ...
            if let Some(best) = results.iter().min_by(|a, b| {
                a.nanoseconds_per_event
                    .partial_cmp(&b.nanoseconds_per_event)
                    .unwrap()
            }) {
                println!(
                    "  最佳配置: {} ({:.1} ns/事件)",
                    best.config_name, best.nanoseconds_per_event
                );
            }
            ...
            if let Some(worst) = results.iter().max_by(|a, b| {
                ...
                println!(
                    "  最差配置: {} ({:.1} ns/事件)",
                    worst.config_name, worst.nanoseconds_per_event
                );
            }
        }
