...
impl<E: EventDataTrait> TimerEventPool<E> {
    ...
    #[inline]
    fn simd_process_chunk(
        &self,
        start_index: usize,
        requests: &[(ConnectionId, E)],
        chunk: &mut [TimerEventData<E>],
    ) {
        let conn_ids = [
            requests[start_index].0,
            requests[start_index + 1].0,
            requests[start_index + 2].0,
            requests[start_index + 3].0,
            requests[start_index + 4].0,
            requests[start_index + 5].0,
            requests[start_index + 6].0,
            requests[start_index + 7].0,
        ];
    ...
    #[inline]
    fn simd_create_chunk(
        &self,
        start_index: usize,
        requests: &[(ConnectionId, E)],
        target_vec: &mut Vec<TimerEventData<E>>,
    ) {
        let conn_ids = [
            requests[start_index].0,
            requests[start_index + 1].0,
        ...
        for k in 0..8 {
            target_vec.push(TimerEventData::new(
                processed_ids[k],
                requests[start_index + k].1.clone(),
            ));
        }
    }

    #[inline]
    fn simd_create_chunk_4(
        &self,
        start_index: usize,
        requests: &[(ConnectionId, E)],
        target_vec: &mut Vec<TimerEventData<E>>,
