...
use crate::timer::{
    TimerTaskStats,
    event::{ConnectionId, TimerEvent, TimerEventId},
    parallel::core::HybridParallelTimerSystem,
    task::{
        commands::{TimerError, TimerTaskCommand},
        types::{
            BatchTimerCancellation, BatchTimerRegistration, BatchTimerResult, SenderCallback,
            TimerCallback, TimerHandle, TimerRegistration,
        },
    },
    wheel::{TimerEntry, TimerEntryId, TimingWheel},
};
...
impl<E: EventDataTrait, C: TimerCallback<E>> HybridTimerTask<E, C> {
    ...
    pub fn new(
        ...
        parallel_threshold: usize,
    ) -> (Self, mpsc::Sender<TimerTaskCommand<E, C>>) {
    ...
    async fn advance_timing_wheel_hybrid(&mut self) {
        ...
        let should_use_parallel =
            batch_size >= self.parallel_threshold && self.stats.total_timers >= 100; // 额外的负载检查
        ...
        if should_use_parallel {
            ...
            match self
                .parallel_system
                .process_timer_batch(expired_timers)
                .await
            {
                Ok(parallel_result) => {
                    trace!(
                    ...
                    self.send_timer_notifications_optimized(notification_data)
                        .await;
                }
                Err(e) => {
                    ...
                    self.process_timers_from_notification_data_optimized(notification_data)
                        .await;
                }
            }
        } else {
    ...
    async fn send_timer_notifications_optimized(
        ...
        if notification_count <= 16 {
            ...
            self.send_notifications_chunked_concurrent(notification_data)
                .await;
        }
    }
    ...
    async fn send_timer_notifications_from_data(
        ...
        let notification_futures: Vec<_> = notification_data
            .into_iter()
            .map(
                |(entry_id, connection_id, event_type, callback_tx, event_data)| async move {
        ...
        debug!(
            total_notifications = notification_count,
            successful_notifications, "Timer notifications sent after parallel processing"
        );
    }
    ...
    async fn process_timers_from_notification_data(
        ...
        if processed_count > 1 {
            debug!(
                processed_count,
                "Sequential batch processed expired timers from fallback"
            );
        }
    }
    ...
    async fn process_timers_sequential(&mut self, expired_timers: Vec<TimerEntry<E, C>>) {
        ...
        for entry in expired_timers {
            ...
            if let Err(_e) = entry
                .event
                .callback
                .on_timeout(entry.event.data.clone())
                .await
            {
                warn!(entry_id, connection_id, "Failed to send timer event");
            } else {
    ...
    async fn handle_command(&mut self, command: TimerTaskCommand<E, C>) -> bool {
        ...
        match command {
            TimerTaskCommand::RegisterTimer {
                registration,
                response_tx,
            } => {
            ...
            TimerTaskCommand::BatchRegisterTimers {
                batch_registration,
                response_tx,
            } => {
            ...
            TimerTaskCommand::BatchCancelTimers {
                batch_cancellation,
                response_tx,
            } => {
            ...
            TimerTaskCommand::ClearConnectionTimers {
                connection_id,
                response_tx,
            } => {
    ...
    async fn register_timer(
        ...
        self.entry_to_connection
            .insert(entry_id, registration.connection_id);
    ...
    async fn batch_register_timers(
        &mut self,
        batch_registration: BatchTimerRegistration<E, C>,
    ) -> BatchTimerResult<TimerHandle<E, C>> {
        ...
        for (entry_id, (connection_id, _delay)) in entry_ids.iter().zip(registration_data.iter()) {
            ...
            if let Some(connection_entries) = self
    ...
    /// 批量取消定时器
    /// Batch cancel timers
    fn batch_cancel_timers(
        &mut self,
        batch_cancellation: BatchTimerCancellation,
    ) -> BatchTimerResult<bool> {
    ...
    fn clear_connection_timers(&mut self, connection_id: ConnectionId) -> usize {
        ...
        if let Some(entry_ids) = self.connection_timers.remove(&connection_id) {
            ...
            debug!(connection_id, count, "Cleared all timers for connection");
        }
...
impl<E: EventDataTrait, C: TimerCallback<E>> HybridTimerTaskHandle<E, C> {
    ...
    pub async fn clear_connection_timers(
        &self,
...
pub fn start_hybrid_timer_task<E: EventDataTrait, C: TimerCallback<E>>()
-> HybridTimerTaskHandle<E, C> {
...
mod tests {
    ...
    use crate::timer::task::types::{
        BatchTimerCancellation, BatchTimerRegistration, TimerRegistration,
    };
    ...
    #[allow(unused)]
    fn create_test_timer_entries<E: EventDataTrait>(
        count: usize,
    ) -> Vec<TimerEntry<E, SenderCallback<E>>> {
    ...
    fn create_test_timer_registrations<E: EventDataTrait>(
        count: usize,
    ) -> Vec<TimerRegistration<E, SenderCallback<E>>> {
    ...
    async fn test_hybrid_timer_basic_functionality() {
        ...
        let (task, command_tx) =
            HybridTimerTask::<TestTimeoutEvent, SenderCallback<TestTimeoutEvent>>::new_default();
    ...
    async fn test_hybrid_timer_batch_operations() {
        ...
        println!(
            "批量取消100个定时器耗时: {:.3}ms",
            cancellation_time.as_secs_f64() * 1000.0
        );
    ...
    async fn test_hybrid_timer_connection_management() {
        ...
        // 测试清理特定连接的定时器
        let cleared_count = handle
            .clear_connection_timers(5)
            .await
            .expect("Failed to clear connection timers");
        println!("连接5清理的定时器数: {}", cleared_count);
        assert_eq!(cleared_count, 10);

        // 验证统计信息更新
        let stats_after_clear = handle
            .get_stats()
            .await
    ...
    async fn test_comprehensive_hybrid_benchmark() {
        ...
        let (task, command_tx) =
            HybridTimerTask::<TestTimeoutEvent, SenderCallback<TestTimeoutEvent>>::new(2048, 32);
        let handle = HybridTimerTaskHandle::new(command_tx.clone());
        ...
        for (batch_size, scenario) in benchmark_cases {
            ...
            for _ in 0..5 {
                ...
                let warmup_batch = BatchTimerRegistration {
                    registrations: warmup_registrations,
                };
                let _ = handle.batch_register_timers(warmup_batch).await;
            }

            // 基准测试
            let benchmark_iterations = if batch_size <= 32 {
                1000
            ...
            // 计算统计数据
            let avg_reg_duration = registration_durations.iter().sum::<Duration>()
                / registration_durations.len() as u32;
            let min_reg_duration = *registration_durations.iter().min().unwrap();
            let max_reg_duration = *registration_durations.iter().max().unwrap();

            let avg_cancel_duration = cancellation_durations.iter().sum::<Duration>()
                / cancellation_durations.len() as u32;
            ...
            println!(
                "    平均延迟: {:.3}µs",
                avg_reg_duration.as_secs_f64() * 1_000_000.0
            );
            println!(
                "    最小延迟: {:.3}µs",
                min_reg_duration.as_secs_f64() * 1_000_000.0
            );
            println!(
                "    最大延迟: {:.3}µs",
                max_reg_duration.as_secs_f64() * 1_000_000.0
            );
            ...
            println!(
                "    平均延迟: {:.3}µs",
                avg_cancel_duration.as_secs_f64() * 1_000_000.0
            );
            println!(
                "    最小延迟: {:.3}µs",
                min_cancel_duration.as_secs_f64() * 1_000_000.0
            );
            println!(
                "    最大延迟: {:.3}µs",
                max_cancel_duration.as_secs_f64() * 1_000_000.0
            );
    ...
    async fn test_hybrid_timer_stress_test() {
        ...
        let (task, command_tx) =
            HybridTimerTask::<TestTimeoutEvent, SenderCallback<TestTimeoutEvent>>::new(4096, 64);
        let handle = HybridTimerTaskHandle::new(command_tx);
        ...
        println!(
            "启动 {} 个并发任务，每个任务处理 {} 个定时器",
            concurrent_tasks, timers_per_task
        );
        ...
        for task_id in 0..concurrent_tasks {
            ...
            let task_handle = tokio::spawn(async move {
                ...
                if half_count > 0 {
                    let entry_ids: Vec<_> = task_timers[..half_count]
                        .iter()
                        .map(|h| h.entry_id)
                        .collect();
                    let batch_cancellation = BatchTimerCancellation { entry_ids };
        ...
        println!(
            "平均吞吐量: {:.0} timers/sec",
            total_processed as f64 / stress_duration.as_secs_f64()
        );
    ...
    async fn test_hybrid_timer_memory_efficiency() {
        ...
        println!(
            "内存增长: {} bytes",
            memory_after_reg.saturating_sub(memory_start)
        );
        println!("成功注册: {}", batch_result.success_count());
        ...
        println!(
            "取消完成耗时: {:.3}ms",
            cancel_duration.as_secs_f64() * 1000.0
        );
        println!(
            "取消后内存: {} bytes (相对于注册后: {})",
            memory_after_cancel,
    ...
    async fn test_hybrid_timer_threshold_behavior() {
        ...
        for (threshold, name) in threshold_tests {
            ...
            let (task, command_tx) = HybridTimerTask::<
                TestTimeoutEvent,
                SenderCallback<TestTimeoutEvent>,
            >::new(1024, threshold);
            let handle = HybridTimerTaskHandle::new(command_tx);
            ...
            for batch_size in test_sizes {
                ...
                let mode = if batch_size >= threshold {
                    "并行"
                } else {
                    "顺序"
                };
