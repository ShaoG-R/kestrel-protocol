...
mod tests {
    ...
    async fn test_strategy_selection() {
        ...
        assert_eq!(
            system.choose_optimal_strategy(100),
            OptimalParallelStrategy::SIMDOnly
        );
        assert_eq!(
            system.choose_optimal_strategy(1000),
            OptimalParallelStrategy::SIMDOnly
        );
        assert_eq!(
            system.choose_optimal_strategy(10000),
    ...
    async fn test_strategy_selection_comprehensive() {
        ...
        // 边界值测试
        assert_eq!(
            system.choose_optimal_strategy(0),
            OptimalParallelStrategy::SIMDOnly
        );
        assert_eq!(
            system.choose_optimal_strategy(256),
            OptimalParallelStrategy::SIMDOnly
        );
        assert_eq!(
            system.choose_optimal_strategy(257),
            OptimalParallelStrategy::SIMDOnly
        );
        assert_eq!(
            system.choose_optimal_strategy(1023),
            OptimalParallelStrategy::SIMDOnly
        );
        assert_eq!(
            system.choose_optimal_strategy(1024),
            OptimalParallelStrategy::FullHybrid
        );
        assert_eq!(
            system.choose_optimal_strategy(4095),
            OptimalParallelStrategy::FullHybrid
        );
        assert_eq!(
            system.choose_optimal_strategy(4096),
            OptimalParallelStrategy::FullHybrid
        );
        assert_eq!(
            system.choose_optimal_strategy(100000),
            OptimalParallelStrategy::FullHybrid
        );
    }
    ...
    async fn test_rayon_executor() {
        ...
        let result = executor
            .parallel_process_with_simd(test_entries, &mut simd_processor)
            .await;
        ...
        println!(
            "Rayon + SIMD parallel processing 1000 timers took: {:?}",
            duration
        );
    ...
    async fn test_error_handling() {
        ...
        let empty_batch = vec![];
        let result = system
            .process_timer_batch::<SenderCallback<TimeoutEvent>>(empty_batch)
            .await;
        assert!(result.is_ok());
        let processed = result.unwrap();
    ...
    async fn test_system_statistics() {
        ...
        println!(
            "Processed {} batches, SIMD-only: {}, Full-hybrid: {}",
    ...
    async fn test_async_overhead_optimization_effectiveness() {
        ...
        for (batch_size, scenario) in optimization_test_cases {
            ...
            let should_bypass = optimized_system
                .mode_selector
                .should_bypass_async(batch_size);
        ...
        println!(
            "  整体吞吐量: {:.0} ops/sec",
            stats.overall_throughput_ops_per_sec
        );
        println!(
            "  平均处理时间: {:.3}µs",
    ...
    async fn test_comprehensive_optimization_benchmark() {
        ...
        for (batch_size, scenario) in benchmark_cases {
            ...
            let throughput = if total_benchmark_time.as_secs_f64() > 0.0 {
                (batch_size as f64 * benchmark_iterations as f64)
            ...
            println!(
                "    最小延迟: {:.3}µs",
                min_duration.as_secs_f64() * 1_000_000.0
            );
            println!(
                "    最大延迟: {:.3}µs",
                max_duration.as_secs_f64() * 1_000_000.0
            );
            ...
            println!(
                "    分配效率: {}",
                if avg_memory_allocs <= 1.0 {
                    "优秀"
                } else if avg_memory_allocs <= 2.0 {
                    "良好"
                } else {
                    "需改进"
                }
        ...
        println!(
            "  整体吞吐量: {:.0} ops/sec",
            final_stats.overall_throughput_ops_per_sec
        );
        println!(
            "  平均处理时间: {:.3}µs",
            final_stats.avg_processing_time_ns / 1000.0
        );
