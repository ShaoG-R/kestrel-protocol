//! 全局定时器任务测试（完全泛型版本）
//! Global timer task tests (Fully generic version)
//!
//! 本模块包含适应完全泛型系统的定时器任务测试套件。
//!
//! This module contains timer task test suites adapted for the fully generic system.

#[cfg(test)]
mod tests {
    use super::super::{
        commands::TimerError,
        types::{BatchTimerCancellation, BatchTimerRegistration, SenderTimerRegistration},
    };
    ...
    async fn test_timer_task_creation() {
        ...
        let (_task, _command_tx) =
            HybridTimerTask::<TimeoutEvent, SenderCallback<TimeoutEvent>>::new_default();
    ...
    async fn test_register_and_cancel_timer() {
        let handle = start_hybrid_timer_task::<TimeoutEvent, SenderCallback<TimeoutEvent>>();
    ...
    async fn test_multiple_timer_types() {
        ...
        for i in 0..timeout_types.len() {
            ...
            .await
            {
                ...
                Err(_) => {
                    panic!(
                        "Timeout waiting for event {} after receiving {} events",
                        i,
                        received_events.len()
                    );
                }
        ...
        for timeout_type in &timeout_types {
            assert!(
                received_events.contains(timeout_type),
                "Missing timeout event: {:?}, received: {:?}",
                timeout_type,
                received_events
            );
        }
    ...
    async fn test_timer_replacement() {
        ...
        // 注册一个长时间的定时器
        let registration1 = SenderTimerRegistration::with_sender(
            1,
        ...
        let registration2 = SenderTimerRegistration::with_sender(
            1,
            Duration::from_millis(100),
            TimeoutEvent::IdleTimeout,
            callback_tx.clone(),
        ...
        // 等待短时间定时器到期
        let event_data = tokio::time::timeout(Duration::from_millis(200), callback_rx.recv())
            .await
            .unwrap()
            .unwrap();
        ...
        let result = tokio::time::timeout(Duration::from_millis(100), callback_rx.recv()).await;
    ...
    async fn test_timer_performance() {
        ...
        println!(
            "注册{}个定时器耗时: {:?}",
            timer_count, registration_duration
        );
        ...
        println!(
            "取消{}个定时器耗时: {:?}",
            timer_count, cancellation_duration
        );
    ...
    async fn test_batch_timer_performance() {
        ...
        println!(
            "批量处理{}个定时器耗时: {:?}",
            received_count, batch_duration
        );
        if received_count > 0 {
            println!(
                "平均每个定时器处理时间: {:?}",
                batch_duration / received_count as u32
            );
        }
        ...
        assert!(
            received_count >= timer_count * 9 / 10,
            "至少90%的定时器应该被处理，实际: {}/{}",
            received_count,
            timer_count
        );
        ...
        println!(
            "handle_timer_events_percent: {}",
            received_count as f64 / timer_count as f64
        );
        ...
        if received_count > 0 {
            ...
            assert!(
                avg_per_timer < Duration::from_millis(10),
                "每个定时器处理时间过长: {:?}",
                avg_per_timer
            );
        }
    ...
    async fn test_cache_performance() {
        ...
        // 性能断言：多次查询应该受益于缓存
        assert!(
            stats_duration < Duration::from_millis(10),
            "统计查询性能不达标"
        );
    ...
    async fn test_batch_timer_performance_comparison() {
        ...
        println!(
            "单个注册{}个定时器耗时: {:?}",
            timer_count, individual_registration_duration
        );
        ...
        println!(
            "单个取消{}个定时器耗时: {:?}",
            timer_count, individual_cancellation_duration
        );
        ...
        println!(
            "批量注册{}个定时器耗时: {:?}",
            timer_count, batch_registration_duration
        );
        ...
        // 批量取消
        let start_time = Instant::now();
        let entry_ids: Vec<_> = batch_result
            .successes
            .into_iter()
            .map(|h| h.entry_id)
            .collect();
        ...
        println!(
            "批量取消{}个定时器耗时: {:?}",
            timer_count, batch_cancellation_duration
        );
        ...
        if batch_nanos_per_timer <= 1000 {
            println!(
                "🎉 达成目标！批量操作每定时器耗时: {}纳秒 (< 1微秒)",
                batch_nanos_per_timer
            );
        } else {
            println!(
                "⚠️  距离目标还有差距，当前: {}纳秒，目标: <1000纳秒",
                batch_nanos_per_timer
            );
        }
    ...
    async fn test_global_timer_task_error_recovery() {
        ...
        let result = handle
            .batch_cancel_timers(empty_batch_cancel)
            .await
            .unwrap();
        assert_eq!(result.success_count(), 0);
        assert_eq!(result.failure_count(), 0);
    ...
    async fn test_high_load_batch_processing() {
        ...
        println!(
            "  ✅ 成功注册{}个定时器，耗时{:?}",
            batch_size, registration_duration
        );
        ...
        println!(
            "  ✅ 统计信息正确：{}个定时器，{}个连接",
            stats.total_timers, stats.active_connections
        );
        ...
        println!(
            "  ✅ 成功取消{}个定时器，耗时{:?}",
            batch_size, cancellation_duration
        );
        ...
        for iteration in 0..iterations {
            // 注册
            let mut batch_registration =
                BatchTimerRegistration::with_capacity(batch_size_per_iteration);
    ...
    async fn test_concurrent_access_safety() {
        ...
        println!(
            "  ✅ 并发操作完成：期望{}个剩余定时器，实际{}个",
            expected_remaining, stats.total_timers
        );
        ...
        for task_id in 0..batch_tasks {
            ...
            let task_handle = tokio::spawn(async move {
                ...
                let entry_ids: Vec<_> = result.successes.into_iter().map(|h| h.entry_id).collect();
                let batch_cancellation = BatchTimerCancellation::new(entry_ids);
                let cancel_result = handle_clone
                    .batch_cancel_timers(batch_cancellation)
                    .await
                    .unwrap();
                assert_eq!(cancel_result.success_count(), batch_size);
