...
async fn test_global_timer_integration() {
    ...
    // 创建定时器管理器（事件驱动接收）
    let connection_id = 1;
    let (mut timer_manager, mut timer_rx) =
        crate::core::endpoint::timing::TimerManager::new_with_receiver(
            connection_id,
            timer_handle.clone(),
        );

    // 注册一个短时间的定时器
    let result = timer_manager
        .register_timer(TimeoutEvent::IdleTimeout, Duration::from_millis(100))
        .await;
...
async fn test_timer_cancellation() {
    ...
    // 注册定时器
    timer_manager
        .register_timer(TimeoutEvent::IdleTimeout, Duration::from_millis(200))
        .await
        .unwrap();
...
async fn test_multiple_timer_types() {
    ...
    assert!(
        received.contains(&TimeoutEvent::IdleTimeout),
        "Missing IdleTimeout event"
    );
    assert!(
        received.contains(&TimeoutEvent::PathValidationTimeout),
        "Missing PathValidationTimeout event"
    );
...
async fn test_timer_replacement() {
    ...
    // 注册一个长时间定时器
    timer_manager
        .register_timer(TimeoutEvent::IdleTimeout, Duration::from_secs(10))
        .await
        .unwrap();
