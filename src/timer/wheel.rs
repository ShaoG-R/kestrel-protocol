...
mod tests {
    ...
    use crate::timer::event::{TimerEvent, TimerEventData};
    ...
    fn create_test_timer_events(
        count: usize,
    ) -> Vec<TimerEvent<TimeoutEvent, SenderCallback<TimeoutEvent>>> {
        (0..count)
    ...
    fn test_timing_wheel_creation() {
        let wheel: TimingWheel<TimeoutEvent, SenderCallback<TimeoutEvent>> =
            TimingWheel::new(8, Duration::from_millis(100));
    ...
    fn test_invalid_slot_count() {
        TimingWheel::<TimeoutEvent, SenderCallback<TimeoutEvent>>::new(
            7,
    ...
    fn test_batch_add_timers_small() {
        ...
        let timer_data: Vec<(
            Duration,
            TimerEvent<TimeoutEvent, SenderCallback<TimeoutEvent>>,
        )> = events
            .into_iter()
            .enumerate()
    ...
    fn test_timer_id_uniqueness() {
        ...
        for i in 0..1000 {
            ...
            assert!(
                all_ids.insert(timer_id),
                "Timer ID {} is not unique",
                timer_id
            );
        }
