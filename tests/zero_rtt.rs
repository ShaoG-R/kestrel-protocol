...
async fn test_basic_zero_rtt_connection() {
    ...
    info!(
        "[Server] 成功接收所有数据: 0-RTT + 后续消息 ({} bytes)",
        all_data.len()
    );
...
async fn test_zero_rtt_packet_coalescing() {
    ...
    info!(
        "[Client] 发送大量0-RTT数据 ({} bytes)...",
        large_initial_data.len()
    );
    let client_stream = client
        .connect_with_config(server_addr, Box::new(config), Some(initial_data))
        .await
...
async fn test_zero_rtt_data_integrity() {
    ...
    info!(
        "[Server] 数据完整性验证通过，所有 {} bytes 数据正确",
        received_data.len()
    );
    info!("--- 0-RTT数据完整性测试通过 ---");
}
...
async fn test_mixed_zero_rtt_and_regular_connections() {
    ...
    info!(
        "--- 测试混合连接场景 ({} 0-RTT + {} 普通) ---",
        ZERO_RTT_CLIENTS, REGULAR_CLIENTS
    );
